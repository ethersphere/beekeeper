...
func (p *PostageService) CreatePostageBatch(ctx context.Context, gasPrice, amount int64, depth uint64, label string) (string, error) {
	url := fmt.Sprintf("/%s/stamps/%d/%d?label=%s", apiVersion, amount, depth, label)
	var resp postageResponse
	h := http.Header{}
	h.Add("Gas-Price", strconv.FormatInt(gasPrice, 10))
