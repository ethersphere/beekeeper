...
func Check(c bee.Cluster, o Options, pusher *push.Pusher, pushMetrics bool) (err error) {
	...
	for _, sIndex := range s1Indexes {
		if err = chaos.NetworkPartition(ctx, o.KubeConfig, chaosCreate, chaosMode, chaosValue, "all", "", o.Namespace, fmt.Sprintf("%s-%d", chaosPodname, sIndex), "bee", "from", chaosDuration, chaosCron); err != nil {
			return err
		}
	...
	for _, sIndex := range s1Indexes {
		if err = chaos.NetworkPartition(ctx, o.KubeConfig, chaosDelete, chaosMode, chaosValue, "all", "", o.Namespace, fmt.Sprintf("%s-%d", chaosPodname, sIndex), "bee", "from", chaosDuration, chaosCron); err != nil {
			return err
		}
