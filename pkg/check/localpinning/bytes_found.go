...
func CheckBytesFound(c *bee.Cluster, o Options) error {
	...
	ref, err := node.UploadBytes(ctx, buf, api.UploadOptions{Pin: true})
	if err != nil {
	...
	for i := 0; i < o.StoreSizeDivisor; i++ {
		...
		a, err := node.UploadBytes(ctx, buf, api.UploadOptions{Pin: false})
		if err != nil {
			return fmt.Errorf("node %s: upload bytes: %w", pivotNode, err)
		}
	...
	// cleanup
	for _, a := range addrs {
		if err := node.UnpinChunk(ctx, a); err != nil {
