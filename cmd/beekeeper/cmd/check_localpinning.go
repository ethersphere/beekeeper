...
func (c *command) initCheckLocalPinningChunk() *cobra.Command {
	...
	cmdChunk.Flags().BoolVar(&startCluster, optionNameStartCluster, false, "start new cluster")
	cmdChunk.Flags().StringVar(&clusterName, optionNameClusterName, "beekeeper", "cluster name")
	cmdChunk.Flags().IntVarP(&bootnodeCount, optionNameBootnodeCount, "b", 0, "number of bootnodes")
	cmdChunk.Flags().IntVarP(&nodeCount, optionNameNodeCount, "c", 1, "number of nodes")
	cmdChunk.Flags().StringVar(&image, optionNameImage, "ethersphere/bee:latest", "Bee Docker image")
	cmdChunk.PersistentFlags().BoolVar(&persistence, optionNamePersistence, false, "use persistent storage")
	cmdChunk.PersistentFlags().StringVar(&storageClass, optionNameStorageClass, "local-storage", "storage class name")
	...
	cmdChunk.PersistentFlags().StringVar(&additionalStorageRequest, optionNameAdditionalStorageRequest, "34Gi", "storage request")
	cmdChunk.Flags().StringArrayVar(&imagePullSecrets, optionNameImagePullSecrets, []string{"regcred"}, "image pull secrets")
