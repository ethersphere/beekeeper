...
func (c *command) initStakeDeposit() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) (err error) {
			...
			clients, nodes, err := c.getStakeTargets(cmd)
			if err != nil {
				return err
			}

			parallel, nodeCount, err := c.getStakeParallelConfig(cmd, clients, nodes)
			if err != nil {
				return err
			}
			...
			depositOp := func(nodeName string, client *bee.Client, node node.Node) error {
			...
			errorCount := c.executeStakeOperation(clients, nodes, parallel, depositOp)
...
func (c *command) initStakeGet() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			getOp := func(nodeName string, client *bee.Client, node node.Node) error {
				ctx := context.Background()
				...
				if len(nodes) > 0 {
					...
					// Cluster mode
					fmt.Printf("Getting stake from node %s...\n", nodeName)
					stakeAmount, err := client.GetStake(ctx)
					if err != nil {
						return err
					}
					fmt.Printf("Node %s: %s WEI staked\n", nodeName, stakeAmount.String())
				}
				return nil
			}

			errorCount := c.executeStakeOperation(clients, nodes, parallel, getOp)
...
func (c *command) initStakeWithdraw() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			withdrawOp := func(nodeName string, client *bee.Client, node node.Node) error {
				ctx := context.Background()
				...
				if len(nodes) > 0 {
					...
					// Cluster mode
					fmt.Printf("Withdrawing stake from node %s...\n", nodeName)
					txHash, err := client.MigrateStake(ctx)
					if err != nil {
						return err
					}
					fmt.Printf("Successfully withdrew stake from node %s, transaction: %s\n", nodeName, txHash)
			...
			errorCount := c.executeStakeOperation(clients, nodes, parallel, withdrawOp)
...
// getStakeTargets handles node discovery for both namespace and cluster modes
func (c *command) getStakeTargets(cmd *cobra.Command) (map[string]*bee.Client, node.NodeList, error) {
	namespace, err := cmd.Flags().GetString(optionNameNamespace)
	if err != nil {
		return nil, nil, fmt.Errorf("error reading namespace flag: %w", err)
	}

	clusterName, err := cmd.Flags().GetString(optionNameClusterName)
	if err != nil {
		return nil, nil, fmt.Errorf("error reading cluster-name flag: %w", err)
	}

	if clusterName == "" && namespace == "" {
		return nil, nil, fmt.Errorf("either cluster-name or namespace must be provided")
	}
	...
	if namespace != "" {
		...
		if err != nil {
			return nil, nil, fmt.Errorf("error reading label-selector flag: %w", err)
		}
		...
		if err != nil {
			return nil, nil, fmt.Errorf("getting nodes: %w", err)
		}
		...
		if err != nil {
			return nil, nil, fmt.Errorf("failed to setup cluster %s: %w", clusterName, err)
		}

		allClients, err := cluster.NodesClients(ctx)
		if err != nil {
			return nil, nil, fmt.Errorf("failed to get node clients: %w", err)
		}

		nodeGroups, err := cmd.Flags().GetStringSlice(optionNameNodeGroups)
		if err != nil {
			return nil, nil, fmt.Errorf("error reading node-groups flag: %w", err)
		}
...
// getStakeParallelConfig handles parallel configuration and validation
func (c *command) getStakeParallelConfig(cmd *cobra.Command, clients map[string]*bee.Client, nodes node.NodeList) (int, int, error) {
	...
	return parallel, nodeCount, nil
}

// addStakeCommonFlags adds common flags to stake commands
func (c *command) addStakeCommonFlags(cmd *cobra.Command) {
...
// stakeOperation defines the signature for stake operations
type stakeOperation func(nodeName string, client *bee.Client, node node.Node) error

// executeStakeOperation runs the given operation in parallel across all target nodes
func (c *command) executeStakeOperation(clients map[string]*bee.Client, nodes node.NodeList, parallel int, operation stakeOperation) int {
	var errorCount int
	var mu sync.Mutex
	...
	if len(nodes) > 0 {
		// Namespace mode: iterate over nodes
		for _, n := range nodes {
			wg.Add(1)
		...
		// Cluster mode: iterate over clients
		for nodeName, client := range clients {
			...
			go func(name string, cl *bee.Client) {
				...
				err := operation(name, cl, node.Node{})
				if err != nil {
					mu.Lock()
					errorCount++
					mu.Unlock()
